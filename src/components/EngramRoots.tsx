...
export const EngramRoots: React.FC = () => {
    ...
    return (
      <div key={node.id} className="flex flex-col">
        <div className={`flex items-center gap-2 py-1 px-2 border-l-2 transition-colors ${
          node.isActiveLayer ? 'border-black bg-gray-100 font-bold' : 
          node.isAncestor ? 'border-gray-400 bg-gray-50 text-gray-500' : 'border-transparent text-gray-400'
        }`} style={{ marginLeft: depth * 12 }}>
          <div className={`w-1.5 h-1.5 rounded-full ${node.isLadder ? 'bg-blue-500' : 'bg-gray-300'}`} />
  ...
  return (
    <div className="w-64 border-r-4 border-black bg-white flex flex-col z-40 h-full overflow-hidden shrink-0">
      <div className="p-2 px-3 flex items-center gap-2 border-b border-gray-100 bg-gray-50">
        <GitBranch size={12} className="opacity-50" />
        <span className="text-[10px] font-black uppercase tracking-widest opacity-50">Engram Roots</span>
      </div>
      <div className="flex-grow overflow-y-auto p-2 font-mono thin-scrollbar">
        {fullTree.map(node => renderTreeNode(node))}
        {fullTree.length === 0 && (
          <div className="text-[9px] text-gray-400 italic p-2 uppercase text-center mt-4">No roots established</div>
        )}
      </div>
    </div>
  );
